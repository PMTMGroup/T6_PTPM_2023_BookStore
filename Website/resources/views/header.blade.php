...
<form action="{{route('login.post.index')}}" method="post">
    @csrf
<div class="box-login" id="box-login">
    ...
    <h4>Chưa có tài khoản ? &nbsp;<a href="{{route('register.index')}}" style="color:#FFC100">Đăng kí ngay</a></h4>
</form>
    @else
    <h3>Xin chào!</h3>
    <h3 style="color:#FFC100">{{$tenThanhVien}}</h3>
    <form action="{{route('logout.post.index')}}" method="post">
